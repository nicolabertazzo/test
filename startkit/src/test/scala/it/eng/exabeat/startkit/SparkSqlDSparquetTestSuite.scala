...
/**
  ...
  * @version 25/03/16
  */
class SparkSqlDSparquetTestSuite extends FlatSpec with Matchers with ResourceAccessor with LocalSqlSparkContext with TempDirSupport {
  ...
  it should "write a parquet Person file" in {
    ...
    personList.toDS().toDF().write.parquet(outputFile)
    val persons = sqlContext.read
      .parquet(outputFile).as[Person]
  ...
  it should "read a parquet Person file" in {
    import sqlContext.implicits._
    val persons = sqlContext.read
      .parquet(getRelativeResourcePath()).as[Person]
    persons.collect() should contain theSameElementsAs(personList)
  }
}
